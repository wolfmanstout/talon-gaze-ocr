...
def test_outside_viewport_match_ratio_full_image_viewport_returns_zero():
    # When the viewport spans the entire captured image there is no outside
    # region to compare, so the function must report no match rather than a
    # spurious perfect score that would let the cache be reused without
    # evidence the screen is unchanged.
    a = np.zeros((10, 10, 3), dtype=np.uint8)
    b = np.full((10, 10, 3), 255, dtype=np.uint8)
    viewport = BoundingBox(0, 0, 10, 10)

    assert outside_viewport_match_ratio(a, b, viewport) == 0.0
    # Oversized viewport (clipped to the image bounds) behaves the same.
    assert outside_viewport_match_ratio(a, b, BoundingBox(0, 0, 20, 20)) == 0.0
