...
mod.setting(
    ...
    type=bool,
    default=True,
    desc="If true, use EasyOCR HTTP server backend instead of other backends. Server starts automatically and loads model once for better performance.",
)
mod.setting(
    "ocr_connect_tracker",
    type=bool,
...
def reload_backend(name, flags):
    ...
    setting_ocr_use_easyocr = settings.get("user.ocr_use_easyocr")
    setting_ocr_use_talon_backend = settings.get("user.ocr_use_talon_backend")
    if setting_ocr_use_easyocr:
